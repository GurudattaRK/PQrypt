...
pub fn derive_password_salt(user_id: &str) -> Result<[u8; ARGON2_SALT_SIZE], CryptoError> {
    ...
    let mut fixed_salt = [0u8; ARGON2_SALT_SIZE];
    fixed_salt[..APP_IDENTIFIER.len().min(32)].copy_from_slice(&APP_IDENTIFIER[..APP_IDENTIFIER.len().min(32)]);
...
pub fn encrypt_key_with_shared_secret(
    ...
    let aes_key_vec = super::symmetric::argon2id_hash(shared_secret, &salt, 32, 10240, 1, 1)?;
