...
import com.pineapple.app.databinding.ActivityKeyExchangeProcessBinding
...
class KeyExchangeProcessActivity : AppCompatActivity() { // Guided UI for 3-message layered key exchange
    ...
    private val pickFolderLauncher = registerForActivityResult( // Launcher for choosing output folder
        ...
        if (result.resultCode == Activity.RESULT_OK) { // Proceed only on confirmation
            ...
            if (treeUri != null) {
                ...
                getSharedPreferences("pineapple_prefs", MODE_PRIVATE)
    ...
    override fun onCreate(savedInstanceState: Bundle?) { // Activity entry point
        ...
        val saved = getSharedPreferences("pineapple_prefs", MODE_PRIVATE).getString("picked_folder_uri", null) // Retrieve saved tree URI
    ...
    private fun launchPickFolder() { // Ask the user to choose a save folder
        val intent = Intent(Intent.ACTION_OPEN_DOCUMENT_TREE).apply { // Build tree picker intent
            ...
            // Try to hint initial location to Documents/Pineapple
            try {
                val hinted = buildPineappleInitialTreeUri() // Construct best-effort initial location
                if (hinted != null) {
    ...
    // Best-effort to hint the picker at /storage/emulated/0/Documents/Pineapple
    private fun buildPineappleInitialTreeUri(): Uri? { // Suggest Documents/Pineapple as initial folder
        return try {
            ...
            val pineapple = File(docs, "Pineapple") // Documents/Pineapple
            if (!pineapple.exists()) pineapple.mkdirs() // Create if missing

            val authority = "com.android.externalstorage.documents" // Documents provider authority
            val docId = "primary:Documents/Pineapple" // Tree doc ID
