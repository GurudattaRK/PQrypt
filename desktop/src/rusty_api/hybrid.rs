...
use super::asymmetric::{crypto_kyber1024_keypair, crypto_kyber1024_encaps, crypto_kyber1024_decaps, crypto_hqc256_keypair, crypto_hqc256_encaps, crypto_hqc256_decaps, crypto_p521_keypair, crypto_x448_keypair, crypto_x448_shared_secret, crypto_p521_shared_secret, crypto_slh_dsa_keypair, crypto_slh_dsa_sign, crypto_slh_dsa_verify};
...
pub type KyberX448SenderState = ([u8; KYBER_SECRETKEYBYTES], [u8; X448_KEY_SIZE], [u8; X448_KEY_SIZE]);
pub type KyberX448ReceiverState = ([u8; KYBER_SECRETKEYBYTES], [u8; X448_KEY_SIZE]);

pub type HqcP521SenderState = ([u8; HQC256_SECRETKEYBYTES], [u8; P521_KEY_SIZE], [u8; P521_SECRET_SIZE]);
pub type HqcP521ReceiverState = ([u8; HQC256_SECRETKEYBYTES], [u8; P521_SECRET_SIZE]);

pub type HybridSenderState = (KyberX448SenderState, HqcP521SenderState, [u8; SLH_DSA_SECRETKEYBYTES], [u8; SLH_DSA_PUBKEYBYTES]);
pub type HybridReceiverState = (KyberX448ReceiverState, HqcP521ReceiverState, [u8; SLH_DSA_SECRETKEYBYTES], [u8; SLH_DSA_PUBKEYBYTES]);
...
fn unpack_and_verify_keyfile(signed: &[u8]) -> Result<Vec<u8>, CryptoError> {
    if signed.len() < SLH_DSA_HEADER_SIZE {
        return Err(CryptoError::InvalidInput);
    }
    // Header parsing
    if &signed[0..8] != &SLH_DSA_HEADER_MAGIC {
        return Err(CryptoError::InvalidInput);
    }
    ...
    if signed.len() != total_needed {
        return Err(CryptoError::InvalidInput);
    }
    // Enforce expected parameter sizes
    if sig_len != SLH_DSA_SIGNATUREBYTES || pk_len != SLH_DSA_PUBKEYBYTES {
        return Err(CryptoError::InvalidInput);
    }
...
pub fn mlkem_x448_init() -> Result<([u8; KYBER_PUBLICKEYBYTES], KyberX448SenderState), CryptoError> {
    ...
    let sender_state = (sender_kyber_sk, sender_x448_pk, sender_x448_sk);
...
pub fn mlkem_x448_recv(
    ...
    let receiver_state = (receiver_kyber_sk, receiver_x448_sk);
...
pub fn mlkem_x448_snd_final(
    ...
    let (sender_kyber_sk, sender_x448_pk, sender_x448_sk) = sender_state;
...
pub fn mlkem_x448_recv_final(
    ...
    let (receiver_kyber_sk, receiver_x448_sk) = receiver_state;
...
pub fn hqc_p521_init() -> Result<([u8; HQC256_PUBLICKEYBYTES], HqcP521SenderState), CryptoError> {
    ...
    let sender_state = (sender_hqc_sk, sender_p521_pk, sender_p521_sk);
...
pub fn hqc_p521_recv(
    ...
    let receiver_state = (receiver_hqc_sk, receiver_p521_sk);
    Ok((bundled_data, receiver_state))
}
...
pub fn hqc_p521_snd_final(
    ...
    let (sender_hqc_sk, sender_p521_pk, sender_p521_sk) = sender_state;
...
pub fn hqc_p521_recv_final(
    ...
    let (receiver_hqc_sk, receiver_p521_sk) = receiver_state;
...
pub fn pqc_4hybrid_init() -> Result<(Vec<u8>, HybridSenderState), CryptoError> {
    ...
    let hybrid_sender_state = (impl1_sender_state, impl2_sender_state, alice_slh_sk, alice_slh_pk);
...
pub fn pqc_4hybrid_recv(
    ...
    let hybrid_receiver_state = (impl1_receiver_state, impl2_receiver_state, bob_slh_sk, bob_slh_pk);
...
pub fn pqc_4hybrid_snd_final(
    ...
    let (impl1_sender_state, impl2_sender_state, alice_slh_sk, alice_slh_pk) = hybrid_sender_state;
    ...
    if content.len() < 4 + impl1_bundle_len {
        return Err(CryptoError::InvalidInput);
    }
    ...
    // Create salt from APP_IDENTIFIER for Argon2
    let salt: [u8; 32] = [
    ...
    // Sign 3.key content with Alice's SLH-DSA
...
pub fn pqc_4hybrid_recv_final(
    ...
    let (impl1_receiver_state, impl2_receiver_state, _bob_slh_sk, _bob_slh_pk) = hybrid_receiver_state;
    ...
    if content.len() < 4 + impl1_final_bundle_len {
        return Err(CryptoError::InvalidInput);
    }
