...
pub fn crypto_x448_keypair() -> Result<([u8; X448_KEY_SIZE], [u8; X448_KEY_SIZE]), CryptoError> {
    ...
    let mut secret_bytes = [0u8; X448_KEY_SIZE];
    super::utils::secure_random_bytes(&mut secret_bytes)?;
    
    let secret = Secret::from_bytes(&secret_bytes)
        .ok_or(CryptoError::KeyGenerationFailed)?;
    let public = PublicKey::from(&secret);
    
    let public_bytes: [u8; X448_KEY_SIZE] = *public.as_bytes();
    let secret_bytes: [u8; X448_KEY_SIZE] = *secret.as_bytes();
...
pub fn crypto_x448_shared_secret(
    ...
    let secret = Secret::from_bytes(my_private)
        .ok_or(CryptoError::InvalidInput)?;
    let public = PublicKey::from_bytes(their_public)
        .ok_or(CryptoError::InvalidInput)?;
    
    let shared_secret = secret.as_diffie_hellman(&public)
        .ok_or(CryptoError::PqcOperationFailed)?;
...
pub fn crypto_p521_shared_secret(public: &[u8; P521_KEY_SIZE], private: &[u8; P521_SECRET_SIZE]) -> Result<[u8; 66], CryptoError> {
    use p521::{PublicKey as P521Public, SecretKey as P521Secret, ecdh::diffie_hellman};
    
    let secret = P521Secret::from_slice(private)
        .map_err(|_| CryptoError::InvalidInput)?;
    let public = P521Public::from_sec1_bytes(public)
        .map_err(|_| CryptoError::InvalidInput)?;
    
    let shared_secret = diffie_hellman(secret.to_nonzero_scalar(), public.as_affine());
